...
login-op:
  ...
  DO +INSTALL_CLI
  COPY aws.env . 
  RUN --secret OP_CONNECT_HOST --secret OP_CONNECT_TOKEN op run --env-file="./aws.env" -- aws sts get-caller-identity
  RUN --secret OP_CONNECT_HOST --secret OP_CONNECT_TOKEN op run --env-file="./aws.env" -- aws s3 ls
